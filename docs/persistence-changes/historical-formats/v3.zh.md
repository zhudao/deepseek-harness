...
查阅 PR #4320 中 V4 写入器变更之前捕获的 V3 持久化类型。系统提示词使用 surface 消息，替换端点使用 `startSeq` / `endSeq`，surface 事件具有显式位置。本快照保留该 V3 检查点的完整声明目录。
...
来源为 `release/session-log-v4` 上 PR #4320 的初始源码树，由日期为 2026-09-16、标题为 `chore(session): initialize V4 release integration PR` 的空提交记录。它包含写入器版本为 3 的 master 源码树，尚未包含 V4 源码变更。目录新鲜度检查通过后，V3 归档命令在写入器推进前捕获了该目录。此检查点既不是 PR 最终源码树，也不是新的产品发布；[发布状态参考](../../session-format-status.zh.md)负责发布证据。

提取的声明来自 `packages/core/session/src/types.ts`、`packages/session/session-persistence-jsonl/src/format.ts`，以及所有第一方 `SessionEventMap` 声明合并及其可达类型。下方源码位置指向该捕获的源码树，不包含行号。
...
- `SessionHeader.version` 解析为字面量 3。逻辑与物理 header 均要求 `isSeeded`；物理 header 没有 `seedLength`。
- `system/message`、`user/message`、`assistant/message` 和 `tool/result` 要求 `surfaceOp`。替换端点为 `startSeq` 与 `endSeq`；Assistant 消息不携带 `sourceEventSeqs`。
- Assistant 流仍内嵌在 message 或 attempt 事件中。系统提示词文本属于 `system/message`，PTC 分派事件使用 `tool/ptc-dispatch-start` 和 `tool/ptc-dispatch`。

[V3 codec](../../../packages/session/session-format-v2-to-v3/src/codec.ts)负责规范事件编码与来源事件范围编码。[V2 到 V3 规范](../../../packages/session/session-format-v2-to-v3/README.zh.md#v2-to-v3-specification)定义历史转换，其[原生 V3 准入规则](../../../packages/session/session-format-v2-to-v3/README.zh.md#native-v3-admission)区分扩展处理与历史来源验证。
...
[完整机器目录](v3.schema.json)包含 60 个根类型和 467 种可达类型。摘要包含所有引用字段；源码名称和路径描述所选的历史源码树。
...
来源：`packages/fs/tool-present/src/types.ts`
...
`pnpm run verify-persistence-catalog` 通过后，`pnpm run verify-persistence-formats --archive 3` 从当前 V3 目录创建了 schema。归档保留每个根及其可达的声明类型、根摘要与不透明声明，并移除源码行号。更早的格式记录保持不变。

目录记录声明字段与可选性，并不覆盖每项 codec 或恢复不变量。外部插件载荷、物理范围编码与运行时关系需要各自的验证器；schema 完整性本身不能确立迁移安全性。

```sh
pnpm run verify-persistence-formats
```

该检查仅使用本工作树验证直到当前写入器的每个格式、schema 完整性、双语声明与生成表格。它不会回放历史应用。维护规则参见[格式索引](README.zh.md)，当前声明参见[当前目录](../../persistence-catalog.zh.md)。
