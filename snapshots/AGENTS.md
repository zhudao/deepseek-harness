...
An owner that must keep a historical generation or retired-tool recording declares its exact `sessionFormat.version` and closed migration `coverage` names in `snapshot.yml`; record and refresh never rewrite that Session fixture. `retired-tools` pins removed capabilities independently of migration coverage, including at the current format version. Owners without this declaration track current-writer output.
