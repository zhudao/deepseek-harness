...
it('grants JIT only to the standalone Node interpreter', async () => {
  const path = root()
  ...
  expect(signMacOSRuntimeCode).toHaveBeenCalledWith(node, expect.any(String), identity,
    join(import.meta.dirname, '../scripts/node-entitlements.plist'))
