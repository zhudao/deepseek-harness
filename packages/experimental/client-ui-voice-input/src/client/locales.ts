...
export const zh = {
  ...
  local: '音频在运行 DSH 的机器上识别。首次准备默认从 Hugging Face 下载模型，该机器需要能访问 Hugging Face 及其模型下载服务。如需代理，请在该机器上配置。',
  cloud: '音频将发送至所选云端服务。',
...
export const en: Record<VoiceKey, string> = {
  ...
  local: 'Audio is recognized on the machine running DSH. First setup downloads models from Hugging Face by default, so that machine must be able to access Hugging Face and its model download services. If a proxy is needed, configure it on that machine.',
